    ...
    <div class="card shadow-sm border-0 rounded-4 p-4">
                ...
                <div class="row">
                    @php
                        $profilData = [
                            ...
                            ['icon' => 'building', 'label' => 'Kota', 'value' => auth()->user()->user->kota ?? '-'],
                            ['icon' => 'map', 'label' => 'Provinsi', 'value' => auth()->user()->user->provinsi ?? '-'],
                            ['icon' => 'mailbox', 'label' => 'Kode Pos', 'value' => auth()->user()->user->kode_pos ?? '-'],
                        ];
                    @endphp
        ...
        <div class="collapse" id="formPerbarui">
            <div class="border rounded-4 p-4 mb-4 bg-light">
                ...
                <form action="{{ route('profil.update') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="kota" class="form-control" value="{{ old('kota', auth()->user()->user->kota ?? '') }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Provinsi</label>
                            <input type="text" name="provinsi" class="form-control" value="{{ old('provinsi', auth()->user()->user->provinsi ?? '') }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Kode Pos</label>
                            <input type="text" name="kode_pos" class="form-control" value="{{ old('kode_pos', auth()->user()->user->kode_pos ?? '') }}">
                        </div>
                    </div>
                ...
                {{-- Form Update Foto --}}
                <hr class="my-4">
                <form action="{{ route('foto.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf @method('PATCH')
                    <div class="mb-3">
